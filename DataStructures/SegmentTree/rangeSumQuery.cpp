...
public:
	...
	long long rangeSumQuery(int node, int query_low, int query_high, int node_low, int node_high){
		...
		return rangeSumQuery(2 * node, query_low, query_high, node_low, left_end_point) 
		+ rangeSumQuery(2 * node + 1, query_low, query_high, left_end_point + 1, node_high);
	}
...
int main(){
	...
	while(queries--){
		...
		if(type == 1){
			...
			int low, high;
			cin >> low >> high;
			cout << st.rangeSumQuery(1, low - 1, high - 1, 0, values - 1) << endl;
		}
	}
	return 0;
}